...
void COutputWnd::SetWindowInfo(const info_notify& info)
{
	COutputList* pWind = nullptr;
	switch (info.the_call)
	{
	case info_notify::output_info:
		pWind = &m_wndOutputDebug; break;
	case info_notify::other_info:
		pWind = &m_wndOutputBuild; break;
	case info_notify::find_info:
		pWind = &m_wndOutputFind; break;
	default:
		return;
	}
	assert(pWind->GetSafeHwnd());
	if (info.bClear)
		pWind->ClearList();
	for (auto pos = 0; ; )
	{
		auto next = info.str.find("\n", pos);
		if (next == std::string::npos)
			break;
		pWind->AddString(pugi::as_wide(info.str.substr(pos, next - pos)).c_str());
		pos = next + 1;
	}
	pWind->SetCurSel(pWind->GetCount() - 1);
}
...
void COutputWnd::SetBuildText(const std::string& str)
{
	...
	for (auto pos = 0; ; )
	{
		...
		if (next == std::string::npos)
			break;
		m_wndOutputBuild.AddString(pugi::as_wide(str.substr(pos, next - pos)).c_str());
		pos = next + 1;
	}
}

void COutputWnd::SetOtherText(const std::string& str)
{
	m_wndOutputDebug.ClearList();
	for (auto pos = 0; ; )
	{
		auto next = str.find("\n", pos);
		if (next == std::string::npos)
			break;
		m_wndOutputDebug.AddString(pugi::as_wide(str.substr(pos, next - pos)).c_str());
		pos = next + 1;
	}
}
