...
struct doc_lib_item
{
	...
	doc_lib_item(const std::wstring& pn, uint64_t ti) :pathname(pn), id(ti), parent(nullptr) {}
	doc_lib_item(const wchar_t* pn, const uint64_t ti, doc_lib_item* pid = nullptr) :pathname(pn), id(ti), parent(pid) {}
};
//gota use list so items do not move
using doc_lib_list = std::list<doc_lib_item>;
